...
program
  ...
  .option(
    "-wc, --warmupSource <string>",
    "the source for the warm up rounds",
    "http://localhost:3000/dbpedia.org/resource/Aachen",
  )
...
const sources =
  options.sources !== undefined ? JSON.parse(options.sources) : undefined;
...
async function getImage() {
  ...
  return new Promise((resolve, reject) =>
    docker.pull(COMUNICA_HDT_IMAGE, {}, (error, result) => {
      if (error) {
        reject(error);
      }
...
async function executeHdtQuery(query, timeout) {
  ...
  const cmd = ["hdt@/data/datadump.hdt", "-q", query, "-t", "stats"];
  const createOptions = {
    Tty: false,
    HostConfig: {
      AutoRemove: true,
